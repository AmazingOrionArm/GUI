...
class RunThread(qc.QThread):
    ...
    def run(self):
        ...
        if(w.spfpath=='' or w.igfpath==''):
            ...
            regionSize=int(w.tbn2.text()) #number of region size
            quaifiedSite=int(w.tbn3.text()) #number of least valid region count
            ...
            for cxt in context:
                ...
                with open(outfile,'w') as of:
                    ...
                    for chromosome in chrs:
                        ...
                        bins = range(0,maxPos,regionSize)
            ...
            promoter = "Promoter"
            igr = "IGR"
            annotation_name1=[exon,intron,utr5,cds,utr3]
            ...
            gene_col.columns=gene_col.iloc[0,:]
            gene_col[gene_col.index.duplicated()]
            gene_col = gene_col[~gene_col.index.duplicated()]
            ...
            gene_body.to_csv(input_gene_name+'_Genebody_bed6.bed', sep='\t',index=False, header=None)
            ...
            gene_promoter.to_csv(input_gene_name+"_Promoter_bed6.bed", sep='\t',index=False, header=None)
            ...
            def dmg(tag):
                ...
                subprocess.call('''bedtools intersect -a %s -b %s >%s '''%(input_gene_name +'_Genebody_bed6.bed',"DMR_"+tag+"_all.txt.bed","DMG_"+tag+"_gene_list.txt"),shell=True)
                subprocess.call('''bedtools intersect -a %s -b %s >%s '''%(input_gene_name +'_Promoter_bed6.bed',"DMR_"+tag+"_all.txt.bed","DMG_"+tag+"_promoter_list.txt"),shell=True)
